...
(() => {
  ...
  async function requestJson(options) {
    ...
    // some builds. Other builds reject that envelope immediately and require
    // the HTTP fetch service; fall back to the compatibility bridge instead of
    // leaving the caller waiting for the outer timeout.
    if (hasElectronFetchBridge()) {
      try {
        return await electronFetchJson(options);
      } catch (error) {
        const message = collapseWs(error?.message || String(error));
        if (!hasCodexPlusBridge() || !/HTTP requests must use the HTTP fetch service|fetch service|message-from-view/i.test(message)) {
          throw error;
        }
        debugLog("native fetch envelope unsupported; falling back to LLM Bridge");
      }
    }
